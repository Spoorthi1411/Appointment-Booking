...
export const ServiceProfile = () => {
  ...
  const { dToken, profileData, getProfileData } = useContext(ServiceContext)
  ...
  // Copy context profile data into local editable state
  useEffect(() => {
    if (profileData) {
      ...
      <div className='flex flex-col lg:flex-row gap-6 max-w-6xl mx-auto mt-6 ml-6'>
        ...
        <div className='flex-1 bg-white/80 backdrop-blur-sm border border-gray-200 rounded-2xl p-8 shadow-xl'>
          ...
          <div className="mb-6">
            ...
            <div className='flex flex-wrap items-center gap-3 text-gray-700'>
              <div className="flex items-center gap-2 bg-[#d86e7c]/10 px-3 py-1 rounded-full">
                ...
                <span className="font-medium">Service: {localProfile.serviceName}</span>
              </div>
              <div className="flex items-center gap-2 bg-[#d86e7c]/10 px-3 py-1 rounded-full">
                <div className="w-2 h-2 bg-[#d86e7c] rounded-full"></div>
                <span className="font-medium">Category: {localProfile.category}</span>
              </div>
            </div>
          </div>
          ...
          <div className="mb-6">
            ...
            {isEdit ? (
              <textarea
                className='text-gray-700 leading-relaxed w-full bg-gray-50 p-4 rounded-xl border border-gray-200'
                value={localProfile.description}
                onChange={(e) =>
                  setLocalProfile(prev => ({ ...prev, description: e.target.value }))
                }
              />
            ) : (
              ...
              <p className="text-xl font-bold text-gray-800 flex items-center">
                ...
                {isEdit ? (
                  <input
                    ...
                    onChange={(e) =>
                      setLocalProfile(prev => ({ ...prev, fees: e.target.value }))
                    }
                  />
                ) : (
                  localProfile.fees
            ...
            <label className="flex items-center gap-3 cursor-pointer group">
              <div className="relative">
                <input
                  ...
                  onChange={() =>
                    isEdit &&
                    setLocalProfile(prev => ({ ...prev, available: !prev.available }))
                  }
                />
          ...
          {isEdit ? (
            <button
              ...
              className="bg-[#d86e7c] hover:bg-[#c55c6b] text-white font-semibold px-8 py-3 rounded-xl shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-300 flex items-center gap-2"
            >
