    ...
    <div className='flex flex-col items-center gap-4 my-16 md:mx-10'>
      ...
      <div className='w-full grid grid-cols-auto gap-2 pt-5 gap-y-6 px-3 sm:px-6'>
        {BusinessList.slice(0,6).map((item,index)=>(
          <div onClick={()=>navigate(`/services/${item.name}`)} className='border  m-2 shadow-lg shadow-slate-500 rounded-xl justify-center items-center overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500' key={index}>
