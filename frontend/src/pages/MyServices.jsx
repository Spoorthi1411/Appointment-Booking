  ...
  const initPay= (order)=>{
    const options ={
      ...
      handler: async (response) => {
        ...
        try {const {data}= await axios.post(backendUrl+'/api/user/verifyRazorpay',response,{headers:{token}})
        if(data.success){
          getUserBookings()
        ...
        {services.map((item,index)=>(
          !item.cancelled && (<div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
            ...
            <div className='flex flex-col gap-2 justify-end'>
              {!item.cancelled && item.payment && <button className='sm:min w-48 py-2 border rounded text-black-500 ' style={{ backgroundColor: '#e39da6ff' }}>Paid</button>}
              {!item.cancelled && !item.payment && <button onClick={()=>appointmentRazorPay(item._id)} className='text-sm text-stone-500 text-center sm:min-w-48 py-2 border hover:bg-slate-800 hover:text-white transition-all duration-300'>Pay Online</button>}
